"""
    This script includes the codes for creating Figure 2 in Section 2.6.2
"""
...
plt.xlabel('r/n')
plt.ylabel('VE')
plt.title('Elliptical model sampling - VE')
...
plt.scatter(c[2:], track[2:, 4], label='Largest leverage simulation', marker='s')
plt.plot(d, 1 + 1 / gamma * A3 * z_numerator_truncated, ls='--', label='Largest leverage theory')
...
plt.xlabel('r/n')
plt.ylabel('PE')
plt.title('Elliptical model sampling - PE')
plt.grid(linestyle='dotted')
plt.legend()
...
plt.grid()
plt.legend()
plt.savefig('VE,n=' + str(n) + ',p=' + str(p) + ',d1=' + str(d1) + ',d2=' + str(d2) + '.png')
# save results
results = pd.DataFrame(track,
                       columns=['VE-sampling', 'PE-sampling', 'RE-sampling', 'VE-largest', 'PE-largest', 'RE-largest'])
results.to_csv('leverage_n=' + str(n) + ',p=' + str(p) + ',d1=' + str(d1) + ',d2=' + str(d2) + '.csv')
sum(z_numerator_truncated >= z_numerator)

plt.hist(leverage_score)
